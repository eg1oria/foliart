...
### Резервные копии из админки

Супер-админ может сам создавать, скачивать, удалять и восстанавливать копии в разделе «Резервные копии» (`/ru/admin/backups`). Обычные администраторы этот раздел не видят: в архиве лежит вся база, включая хеши паролей.

- Копию делает сам backend, без остановки сайта: SQLite снимается через `VACUUM INTO`, затем база и все изображения упаковываются в `foliart-YYYYMMDDTHHMMSSZ.tar.gz` с файлом `.sha256`. Формат тот же, что у `scripts/backup.sh`, поэтому такой архив восстанавливается и через `scripts/restore.sh`.
- Архивы лежат в отдельном volume `backend_backups` (`/app/backups` в контейнере). Хранятся `BACKUP_KEEP` последних (по умолчанию 10, переменная в `.env`), более старые удаляются при создании новой копии.
- Восстановление из админки проверяет контрольную сумму и архив, сохраняет копию текущего состояния с пометкой `-before-restore`, раскладывает файлы рядом с рабочими и перезапускает backend. Подмена базы и изображений выполняется при старте процесса, до подключения к базе; Docker поднимает контейнер заново благодаря `restart: unless-stopped`. Сайт недоступен несколько секунд.
- Копию, у которой набор применённых миграций отличается от текущей базы (сделана до обновления со сменой схемы), админка восстанавливать отказывается — такие копии восстанавливает разработчик через `restore.sh`, который заново прогоняет миграции.

Достать архив из volume на хост, например чтобы восстановить его скриптом:

```bash
cd /opt/foliart
docker compose exec backend ls -lh /app/backups
docker compose cp backend:/app/backups/foliart-YYYYMMDDTHHMMSSZ.tar.gz /tmp/
docker compose cp backend:/app/backups/foliart-YYYYMMDDTHHMMSSZ.tar.gz.sha256 /tmp/
sudo ./scripts/restore.sh /tmp/foliart-YYYYMMDDTHHMMSSZ.tar.gz
```

Результат последнего восстановления из админки записан в `/app/backups/last-restore.json`, подробности — в `docker compose logs backend` (строки `Applying restore` / `Restore ... applied`).
...
## 14. Администраторы админки

Учётные записи админки лежат в базе (`AdminUser`), пароли — в виде scrypt-хешей.

**Первый запуск.** Пока в таблице нет ни одного администратора, backend создаёт
супер-админа из `ADMIN_USERNAME` и `ADMIN_PASSWORD` (пароль — любой непустой)
...
После этого `.env` для входа больше не используется: пароль меняется в самой
админке, раздел «Мой профиль». Менять `ADMIN_PASSWORD` в `.env` бессмысленно —
на вход это уже не влияет.

**Остальные администраторы.** Супер-админ создаёт их в разделе
«Администраторы»: логин, пароль и уровень доступа к каждому разделу — «нет
доступа», «только просмотр» или «полный доступ». Он же может задать любому
администратору новый пароль и удалить учётную запись. Права применяются сразу:
после их изменения активные сессии этого администратора закрываются.

Форма создания подсказывает занятые логины сразу при вводе, умеет сгенерировать
пароль (20 символов) и показать его — скопировать пароль нужно до сохранения,
позже он нигде не отображается. Кнопки над матрицей прав выдают или снимают
доступ ко всем разделам сразу; если не выдан ни один раздел, форма предупредит:
такой администратор войдёт, но увидит только страницу «Нет доступа».

**Логин**: 3–32 символа, латиница **или** кириллица — смешивать алфавиты в одном
логине нельзя, иначе `admin` и `аdmin` с кириллической «а» стали бы двумя
неразличимыми на вид учётными записями. Внутри логина допустимы цифры, точка,
дефис и подчёркивание; регистр не важен, логин приводится к нижнему.

**Пароль**: любой непустой, длиной до 200 символов — ни минимальной длины, ни
требований к составу нет ни в форме, ни в API, ни в скрипте восстановления.

Супер-админ один: его нельзя ограничить в правах и нельзя удалить.

**Если пароль супер-админа потерян.** В контейнере backend есть скрипт
восстановления; отсутствующий логин он создаёт заново как супер-админа.

```bash
cd /opt/foliart
docker compose exec backend npm run admin:reset-password -- admin новый_длинный_пароль
```
